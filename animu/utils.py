...
def verify_query(
    ...
    *episodes: int,
    ...
    has_episodes = len(episodes) > 0 or starting_episode > 0
    ...
    # The episode search query is f'{anime_title} "{ep:02d}"'.  Do not
    # include that suffix in the title similarity calculation: it cannot
    # match the parsed anime title and artificially lowers the Dice score.
    title_query = search_query
    if search_mode == "EPISODE":
        title_query = re.sub(r'''\s+["']\d+["']\s*$''', "", title_query).strip()

    title_match = find_best_match(title_query, targets)
    ...
    if search_mode == "EPISODE":
        ...
        wanted_episode = episodes[0] if episodes else starting_episode
        ...
        # AniList can lag behind the release index for already-published
        # episodes. Missing schedule metadata is unknown, not proof that the
        # torrent is unreleased. Keep validating episode/title/resolution and
        # only enforce the upload-date rule when a schedule row exists.
        schedule_available = page_number != -1
        airing_at_epoch = nodes[page_number]["airingAt"] if schedule_available else 0.0
        air_date_match = (
            ignore_airdate_checks or
            not schedule_available or
            matches_airdate_with_buffer(
                airing_at_epoch,
                pub_epoch
            )
        )
        ...
        if score < 3.88:
            ...
            if best_rating < 0.88:
                failed_checks.append(f"title similarity too low ({best_rating*100:.0f}% < 88.0%)")
        ...
        if episode_range_match:
            ...
            try:
                ...
                verify_range_ok = (start_range == 1 and end_range == last_episode)
            except Exception:
