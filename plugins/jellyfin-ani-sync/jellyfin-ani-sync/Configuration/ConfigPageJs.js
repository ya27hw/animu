var PluginConfig = {
    pluginUniqueId: '2a98f1f7-bd58-450f-90e9-b593eb42dfcc'
};
...
async function initialLoad(common) {
    ...
    async function onAuthorizeButtonClick(common) {
        ...
        if (!clientId || !clientSecret) {
            if (!clientId)
                document.querySelector('#authorizeClientIdError').innerHTML = "Error: Client ID is empty.";

            if (!clientSecret)
                document.querySelector('#authorizeClientSecretError').innerHTML = "Error: Client Secret is empty.";
    ...
    function loadProviderConfiguration(page) {
        ...
        ApiClient.getPluginConfiguration(PluginConfig.pluginUniqueId).then(function (config) {
            ...
            if (config.AniListBearerToken)
                page.querySelector('#aniListBearerTokenInput').value = config.AniListBearerToken;
    ...
    function saveUserConfig(common) {
        ApiClient.getPluginConfiguration(PluginConfig.pluginUniqueId).then(function (config) {
            ...
            config.AniListBearerToken = document.querySelector('#aniListBearerTokenInput').value;
